...
function calculateActivityPoints(type, durationMinutes, intensity) {
    const basePoints = {
        run: 8,
        walk: 5,
        strength: 7,
...
router.get('/users', async (_req, res) => {
    try {
        const users = await User_1.default.find().sort({ totalPoints: -1, createdAt: -1 });
        res.json(users);
    }
    catch (error) {
...
router.get('/teams', async (_req, res) => {
    try {
        const teams = await Team_1.default.find().sort({ totalPoints: -1, createdAt: -1 });
        res.json(teams);
    }
    catch (error) {
...
router.get('/activities', async (_req, res) => {
    try {
        const activities = await Activity_1.default.find().populate('userId').populate('teamId').sort({ completedAt: -1 });
        res.json(activities);
    }
    catch (error) {
...
router.post('/activities', async (req, res) => {
    try {
        ...
        await User_1.default.findByIdAndUpdate(userId, { $inc: { totalPoints: points } });
        if (teamId) {
            await Team_1.default.findByIdAndUpdate(teamId, { $inc: { totalPoints: points } });
        }
